...
fb2 rf = [ FB ([]==)
   ...
   , FB (["round final answer correctly"]==)
    ...
    , FB (anySub 
         ...
         ("In de vraagstelling is gegeven dat de gemiddelde" ++
          " verandering (het hellingsgetal) gelijk is aan " ++ show (y2-y1) ++ 
          ". Let op, dit is geen verhoudingstabel.")
   ...
   , FB (anyElem
        ...
        ("Er in de vraagstelling staat dat er per stap van x "
         ++ show (y2-y1) ++ " bij komt, toch?")
   ...
   , FB (anyElem
        ...
        ("Tussen " ++ show x2 ++ " en " ++ show xv ++ 
         ...
         " = " ++ show (xv-x2) ++ " stappen. Gebruik dit om verder te rekenen.")
   ...
   , FB (anyElem
        ...
        ("Het lijkt er op dat je vergeten bent om vanaf " ++ 
         show y2 ++ " verder te rekenen." )
   ...
   , FB ("calculate y as a sequence" `elem`)
        ...
        ("Je moet toch juist " ++ show x2 ++ " - " ++ show x1 ++ 
         " = " ++ show (x2-x1) ++ " stappen verder rekenen vanaf"
         ++ show y2++ "?")
